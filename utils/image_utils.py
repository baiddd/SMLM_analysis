...
class Merge(object):
    ...
    def __call__(self, images):
        if isinstance(images, collections.Sequence) or isinstance(images, np.ndarray):
            assert all(
                [isinstance(i, np.ndarray) for i in images]
            ), "only numpy array is supported"
    ...
    def __init__(self, *slices, **kwargs):
        ...
        for s in slices:
            if isinstance(s, collections.Sequence):
                ...
                slices_.append(s)
        assert all(
            [isinstance(s, slice) for s in slices_]
        ...
        if isinstance(image, np.ndarray):
            for i in range(image.shape[2]):
                ...
                im.save(
                    os.path.join(
                        self.path,
                        self.prefix + "b" + str(self.counter) + "_c" + str(i) + ".tif",
                    )
                )
                self.counter += 1
        else:
...
class PoissonSubsampling(object):
    ...
    def __call__(self, image):
        if isinstance(self.peak, collections.Sequence):
            peak = num_generator(self.peak, random_state=self.random_state)
        else:
            peak = self.peak
...
class AddGaussianNoise(object):
    ...
    def __call__(self, image):
        if isinstance(self.sigma, collections.Sequence):
            sigma = num_generator(self.sigma, random_state=self.random_state)
        else:
            sigma = self.sigma
...
class MaxScaleNumpy(object):
    ...
    def __call__(self, image):
        ...
        if np.all(mx - mn) == 0:
            return image
        return self.scale[0] + (image - mn) * (self.scale[1] - self.scale[0]) / (
            mx - mn
        )
...
class CenterCropNumpy(object):
    ...
    def __call__(self, img):
        ...
        assert (
            th <= h and tw <= w
        ), "input size ({}, {}) is smaller than crop size({}, {}).".format(h, w, th, tw)
        x1 = int(round((w - tw) / 2.0))
        y1 = int(round((h - th) / 2.0))
        return img[y1 : y1 + th, x1 : x1 + tw, :]


class NpCropNumpy(object):
    """Crops the given numpy array at the center to have a region of
    the given size. size can be a tuple (target_height, target_width)
    or an integer, in which case the target will be of a square shape (size, size)
    """

    def __init__(self, start_coord, size):
        if isinstance(start_coord, numbers.Number):
            self.start_coord = (int(start_coord), int(start_coord))
        else:
            self.start_coord = (int(start_coord[0]), int(start_coord[1]))
        if isinstance(size, numbers.Number):
            self.size = (int(size), int(size))
        else:
            self.size = (int(size[0]), int(size[1]))
    ...
    def __call__(self, img):
        h, w = img.shape[:2]
        th, tw = self.size
        h0, w0 = self.start_coord
        assert (
            th <= h and tw <= w
        ), "input size ({}, {}) is smaller than crop size({}, {}).".format(h, w, th, tw)
        return img[h0 : h0 + th, w0 : w0 + tw, :]
